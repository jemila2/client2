...
const Navbar = () => {
  ...
  const handleLogout = () => {
    logout();
    navigate('#/login'); // Add hash prefix
    setIsProfileDropdownOpen(false);
    setIsMobileMenuOpen(false);
  };
  ...
  // Get the correct dashboard path based on user role - WITH HASH PREFIX
  const getDashboardPath = () => {
    if (!user) return '#/';
    if (isCustomer()) return '#/customer/dashboard';
    if (user.role === 'admin') return '#/admin';
  ...
  // Get the correct profile path based on user role - WITH HASH PREFIX
  const getProfilePath = () => {
    if (!user) return '#/profile';
    if (user.role === 'admin') return '#/profile';
    return `#/profile/${user._id || user.id || user.employeeId}`;
  ...
  return (
    <nav className="bg-blue-800 text-white shadow-md sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          {/* Logo */}
          <Link to="#/" className="flex items-center text-xl font-bold"> {/* Add hash */}
            <span className="text-white">LaundryPro</span>
          </Link>

          {/* Desktop Menu */}
          <div className="hidden md:flex items-center space-x-8">
            {/* Common navigation links - ALL WITH HASH PREFIX */}
            <Link to="#/" className="flex items-center space-x-2 text-blue-100 hover:text-white transition-colors">
              <FiHome className="text-lg" />
              <span>Home</span>
            </Link>
