...
// Import the PostCSS plugins properly
import tailwindcss from '@tailwindcss/postcss';
import autoprefixer from 'autoprefixer';

export default defineConfig(({ mode }) => {
  const env = loadEnv(mode, process.cwd(), '');
  const apiBaseUrl = env.API_BASE_URL || 'https://backend-21-2fu1.onrender.com';
  ...
  return {
    ...
    // ✅ FIXED: Use proper import syntax for PostCSS plugins
    css: {
      postcss: {
        plugins: [
          tailwindcss,
